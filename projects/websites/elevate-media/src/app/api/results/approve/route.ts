...
export async function POST(req: Request) {
  try {
    ...
    if (!doc) return NextResponse.json({ error: "Result not found" }, { status: 404 });
    if (!doc.courseId) return NextResponse.json({ error: "Result has no course; cannot verify" }, { status: 400 });

    const ctx = await resolveCourseContext(supabase, doc.courseId);

    if (!canAct(approver, ctx, as)) {
      if (as === "hod") {
        return NextResponse.json({ error: "Only the HOD can verify this result" }, { status: 403 });
      }
      return NextResponse.json(
        { error: "Only the unit lecturer (or the HOD, by the HOD's powers) can verify this result" },
        { status: 403 },
      );
    }
    ...
    // Releasing makes the result visible to students immediately. Downloads
    // stay locked until the HOD verifies. The unit lecturer side must be
    // verified (by the lecturer or by the HOD on the lecturer's behalf) first.
    if (release === true) {
      if (!nextLecturer) {
        return NextResponse.json(
