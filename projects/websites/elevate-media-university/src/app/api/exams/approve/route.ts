...
export async function POST(req: Request) {
  try {
    ...
    if (!doc.courseId) return NextResponse.json({ error: "Exam has no course; cannot verify" }, { status: 400 });

    const ctx = await resolveCourseContext(supabase, doc.courseId);

    if (!canAct(approver, ctx, as)) {
      if (as === "hod") {
        return NextResponse.json({ error: "Only the HOD can verify this exam" }, { status: 403 });
      }
      return NextResponse.json(
        { error: "Only the unit lecturer (or the HOD, by the HOD's powers) can verify this exam" },
        { status: 403 },
      );
    }
